...
class MultiPulseCountBaseClient(BatchClient):
    """Provides access to pulse count specific control API methods."""
    def reserve(self, amount, wait_for_available=False, available_timeout=60, kind="multipulsecount", flush_interval_seconds=10, flush_at_bitstreams_remaining=25):
        args = {
    ...
    def evaluateBitstreams(self, bitstreams: list[str], serials=None) -> Generator[tuple[str, str, tuple[int, int]]]:
        """Sends bitstream filepaths to be evaluated by iCEFARM. If serials are not specified, bitstreams
        are evaluated on each reserved device. Results are received as (serial, filepath, (ice21, ice25))."""
        if not serials:
