...
from icefarm.client.drivers import VarMaxClient
...
EVALUATE_EACH = True
# Whether to send waveform along with fitness data
SEND_WAVEFORM = True
...
client = VarMaxClient(CONTROL_SERVER, CLIENT_NAME, logger, log_events=EVENT_LOGGING, send_waveform=SEND_WAVEFORM)
...
if EVALUATE_EACH:
    ...
    for bitstream in BITSTREAM_PATHS:
        evaluations.append(VarMaxEvaluation(devices, bitstream))
else:
    ...
    for serial, batches in zip(client.getSerials(), batches):
        ...
        for bitstream in batches:
            evaluations.append(VarMaxEvaluation([serial], bitstream))
...
for result in client.evaluateEvaluations(evaluations):
    print(f"Serial {result.serial}, bitstream {result.evaluation.filepath}, result {result.value}")
